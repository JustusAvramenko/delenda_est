...
ProductionQueue.prototype.AddBatch = function(templateName, type, count, metadata)
{
	...
	if (this.queue.length < MAX_QUEUE_SIZE)
	{
		...
		if (type == "unit")
		{
			...
			if (templateName.contains("hero"))
				cmpPlayer.AddDisabledTemplate(templateName);
...
ProductionQueue.prototype.RemoveBatch = function(id)
{
	...
	for (var i = 0; i < this.queue.length; ++i)
	{
		...
		let templateName = item.unitTemplate;
		if (templateName && templateName.contains("hero"))
			cmpPlayer.RemoveDisabledTemplate(templateName);
