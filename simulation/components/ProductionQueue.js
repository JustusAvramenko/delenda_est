	...
	if (this.queue.length < MAX_QUEUE_SIZE)
	{
		...
		else if (type == "technology")
		{
			...
			var cmpDataTemplateManager = Engine.QueryInterface(SYSTEM_ENTITY, IID_DataTemplateManager);
                       ...
                       if (!template)
                               return;
                       if (!this.GetTechnologiesList().some(tech => tech && (tech == templateName || tech.pair && (tech.top == templateName || tech.bottom == templateName))))
                       {
                               warn("This entity cannot research " + JSON.stringify(templateName) + ". It can only research: " + JSON.stringify(this.GetTechnologiesList()));
                               return;
                       }
                       var cmpPlayer = QueryOwnerInterface(this.entity);			let techCostMultiplier = this.GetTechCostMultiplier();
