...
m.HQ.prototype.init = function(gameState, queues)
{
	...
	this.useGlory = Resources.GetCodes().some(code => code == "glory");
	this.maxStatues = 5;
...
/** Build statues for glory */
m.HQ.prototype.buildStatue = function(gameState, queues)
{
	if (queues.economicBuilding.hasQueuedUnits())
		return;
	if (gameState.getOwnEntitiesByClass("CultStatue", true).length >= this.maxStatues)
		return;

	if (this.canBuild(gameState, "structures/{civ}_statue"))
		queues.economicBuilding.addPlan(new m.ConstructionPlan(gameState, "structures/{civ}_statue"));
}
...
/*	if (this.Config.debug > 1)
	...
	if (this.currentPhase > 1 && gameState.ai.playedTurn % 3 == 0)
	{
		...
		if (!this.saveResources)
		{
			...
			if (this.useGlory)
				this.buildStatue(gameState, queues);
		}
