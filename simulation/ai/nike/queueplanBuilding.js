...
import { getBuiltEntity, getLandAccess, getSeaAccess } from "simulation/ai/nike/entityExtend.js";
import * as mapMask from "simulation/ai/nike/mapMask.js";
import { createObstructionMap } from "simulation/ai/nike/mapModule.js";
...
export class ConstructionPlan extends QueuePlan
{
	...
	start(gameState)
	{
		...
		if (!builder)
		{
			aiWarn("nike error: builder not found when starting construction.");
			Engine.ProfileStop();
			return;
		}
			...
			if (template.hasClass("Granary"))
			{
				for (let j = 0; j < placement.map.length; ++j)
				{
					...
					if (HQ.borderMap.map[j] & mapMask.fullBorder)
						value /= 2;	// we need space around Granary, so disfavor map border
	...
	buildOverseaDock(gameState, template)
	{
		...
		if (!gameState.ai.HQ.navalMap)
			aiWarn("nike.findOverseaLand on a non-naval map??? we should never go there ");
