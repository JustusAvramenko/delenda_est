...
export class BaseManager
{
	...
	setAnchor(gameState, anchorEntity, deserialize = false)
	{
		...
		{
			aiWarn("Error: Nike base " + this.ID + " has been assigned " + ent.templateName() +
				" as anchor.");
		}
		else
		{
	...
	setAnchorlessEntity(gameState, ent, deserialize = false)
	{
		...
		{
			aiWarn(" Error: Nike base " + this.ID + " with access " + this.accessIndex +
			...
			if (this.basesManager.hasActiveBase())
			{
				...
				for (const ent of this.buildings.values())
				{
					...
					if (!bestBase)
					{
						...
						{
							aiWarn("Nike: dock in 'noBase' baseManager. It may be useful to " +
